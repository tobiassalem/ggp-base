...
public class PropNetStateMachine extends StateMachine {
	...
	public void initialize(List<Gdl> description) {
		try {
			...
			Proposition prop = this.propNet.getInitProposition();
			prop.setValue(true);
	...
	private void markBases(MachineState state) {
		Map<GdlSentence, Proposition> bases = this.propNet
				.getBasePropositions();

		for (Map.Entry<GdlSentence, Proposition> e : bases.entrySet()) {
			e.getValue().setValue(false);
		}

		for (GdlSentence sentence : state.getContents()) {
			Proposition p = bases.get(sentence);
			p.setValue(true);
		}
	}
	...
	public List<Move> getLegalMoves(MachineState state, Role role)
		...
		Map<Role, Set<Proposition>> legalPropsMap = this.propNet
		...
		final GdlConstant roleName = role.getName();

		for (Proposition p : legalProps) {

			GdlSentence sentence = p.getName();

			List<GdlTerm> body = sentence.getBody();

			if (roleName.equals(body.get(0))) {
